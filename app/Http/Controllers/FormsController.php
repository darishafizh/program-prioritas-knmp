...
class FormsController extends Controller
{
    ...
    public function index($knmpId, Request $request)
    {
        ...
        $sectionCounts = [
            'A' => ProfileKnmp::where('knmp_id', $knmp->id)->count(),
            'B' => ProgresKnmp::where('knmp_id', $knmp->id)->count(),
            'C' => InformasiResponden::where('knmp_id', $knmp->id)->count(),
            'D' => TanggapanMasyarakat::where('knmp_id', $knmp->id)->count(),
            'E' => TingkatKebahagiaanNelayan::where('knmp_id', $knmp->id)->distinct('responden_id')->count('responden_id'),
            'F' => InformasiUsaha::where('knmp_id', $knmp->id)->count(),
            'G' => InformasiPemasaran::where('knmp_id', $knmp->id)->count(),
            'H' => InformasiPendapatanRumahTangga::where('knmp_id', $knmp->id)->count(),
            'I' => SosialKelembagaan::where('knmp_id', $knmp->id)->count(),
            'J' => BuktiUpload::where('knmp_id', $knmp->id)->count(),
        ];
